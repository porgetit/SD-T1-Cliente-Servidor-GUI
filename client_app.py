...
def main():
    ...
    if "--run-internal" in sys.argv:
        ...
        if python_exe.lower().endswith("python.exe"):
            python_exe = python_exe[:-10] + "pythonw.exe" # Usamos pythonw.exe para evitar que se abra una consola extra en Windows
            
        if not os.path.exists(python_exe): # Si no existe pythonw.exe, usamos python.exe
            python_exe = sys.executable
        ...
        subprocess.Popen([python_exe, __file__, "--run-internal"], 
                         ...
                         close_fds=True) # Lanzamos un proceso hijo desvinculado usando DETACHED_PROCESS
